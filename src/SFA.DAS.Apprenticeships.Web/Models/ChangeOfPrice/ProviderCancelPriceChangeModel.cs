...
public class ProviderCancelPriceChangeModelMapper : IMapper<ProviderCancelPriceChangeModel>
{
    ...
    private static ProviderCancelPriceChangeModel FromGetPendingPriceChangeResponse(GetPendingPriceChangeResponse getPendingPriceChangeResponse)
    {
        ...
        var model = new ProviderCancelPriceChangeModel
        {
            ...
            OriginalTrainingPrice = Parse(pendingPriceChange.OriginalTrainingPrice, nameof(PendingPriceChange.OriginalTrainingPrice)),
            OriginalEndPointAssessmentPrice = Parse(pendingPriceChange.OriginalAssessmentPrice, nameof(PendingPriceChange.OriginalAssessmentPrice)),
    ...
    private static decimal Parse(decimal? value, string propertyName)
    {
        if (!value.HasValue)
        {
